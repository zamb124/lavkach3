...
async def test_update_store(async_client, headers, stores):
    response = await async_client.get(f"/api/basic/store/{stores[0].id.__str__()}")
    ...
    response = await async_client.put(f"/api/basic/store/{stores[0].id.__str__()}", json=update_json, headers=headers['superadmin'])
    assert response.status_code == 200
    data = response.json()
...
async def test_crud_store(async_client, headers, companies):
    ...
    response = await async_client.post("/api/basic/store", json=request_data, headers=headers['superadmin'])
    assert response.status_code == 200
    data = response.json()
    assert data['title'] == request_data['title']
    assert data['company_rel']['id'] == request_data['company_id']
    response = await async_client.delete(f"/api/basic/store/{data['id']}", headers=headers['superadmin'])
    assert response.status_code == 200
