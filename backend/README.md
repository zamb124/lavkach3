...
Микросервисный Фреймворк основанный на FastApi SqlAhlehmy  для быстрого разворачивания сервисов 
функциональностей используя нотации

# Нотации обычными словами
1) Каждый сервис это отдельный сервер fastapi со своей базой данных состоящий из самого {name}*_server и {name}*_config где основные настройки и состоящий из модулей
<pre> - структура 
└──app
├── index.html
├── index.js
...
2) В каждом Модуле(набор функциональностей) должен быть Сервис основанный на BaseService с указанием Pydantic Моделей 
2) 
# todo:
